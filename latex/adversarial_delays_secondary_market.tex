...
\subsection{Possible execution dates}
\label{sec:possible-execution-dates}

Fix a submission time $s$. Define
\[
    \mathcal{J}_s(\boldsymbol{p}_T)
    :=
    \bigl\{s+d_s(a):a\in[0,1],\ s+d_s(a)\le T\bigr\}.
\]
...
\[
    ...
    \left\{
        u\in\{s,\ldots,T\}:
        p_u>
        \max\bigl(\{0\}\cup\{p_v:s\le v<u\}\bigr)
    \right\}.
\]

Indeed, if $u\in\mathcal{J}_s(\boldsymbol{p}_T)$, then one can choose
\[
    \max\bigl(\{0\}\cup\{p_v:s\le v<u\}\bigr)\le a<p_u,
\]
...
For $s\ge2$, define the execution-price exposure
\[
    \omega_s(\boldsymbol{p}_T,\boldsymbol{m}_T)
    :=
    \max_{u\in\mathcal{J}_s(\boldsymbol{p}_T)}
    \bigl|m_u-m_{s-1}\bigr|,
\]
where the maximum is defined to be zero if
$\mathcal{J}_s(\boldsymbol{p}_T)=\varnothing$.
Finally, define
\[
    W_T(\boldsymbol{p}_T,\boldsymbol{m}_T)
    :=
    \sum_{s=2}^T
    \omega_s(\boldsymbol{p}_T,\boldsymbol{m}_T).
\]
...
$(\boldsymbol{p}_T,\boldsymbol{m}_T)$. It does not depend on the learner
or on an auxiliary predictor.
...
$\mathcal{J}_s$. Thus $\omega_s$ is the largest secondary-price movement
to which the offer at $s$ could be exposed.

For example, if $p_t$ is nonincreasing, an offer either executes
immediately or never executes. Hence
$\mathcal{J}_s\subseteq\{s\}$ and
\[
    W_T\le\sum_{s=2}^T|m_s-m_{s-1}|.
\]
Equality holds if $p_s>0$ for every $s=2,\ldots,T$.
...
date $t+d_t(a)$ belongs to $\mathcal{J}_t$. At $t=1$,
\[
...
For every $s=2,\ldots,T$, both the immediate date $s$ and the terminal
date $T$ are possible execution dates, and
\[
    \omega_s=\delta.
\]
Hence
