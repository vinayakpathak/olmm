...
Iteration 19 promotes the two-low closed form as the first reliable base case for this program. For \(h^M,0^{N_0},p,0^{N_1},q,0^L,h\), use LB-045 as a branch-by-branch regression theorem. The \(p<q\) middle case has equal-revenue-like dual weights
\[
\alpha_1=\frac{q-p}{q},\qquad
\alpha_2=\frac{p(h-q)}{hq},\qquad
\beta=\frac ph,
\]
while the boundary cases are capacity or low-1 dominated. The \(p>q\) formula shows that the optimal first-segment high-low mass is simply pushed to \(u^*=\min\{B_0,S_2/h\}\), with a max between low-1 deficit and low-2/high debt after \(B_0\) saturates.

The next active-set target is now more precise:

- prove the KKT support lemma from LB-046 as a standalone proposition in any writeup;
- define exactly what "pool" means: low constraints, quote segments, quote levels, or a combined primal-dual active set;
- test nonmonotone lows such as \(0.4,0.9,0.5\), duplicate lows, zero gaps, \(M>1\), and capacity-saturated two-low branches;
- require every proposed pooling or nested-allocation algorithm to emit exact primal masses, dual weights, and pressure values \(\psi_i\);
- keep the three-low certificate
\[
0,\ 0.2,\ 0^8,\ 0.3,\ 0,\ 0.4,\ 1,\qquad z=291/100
\]
as a regression test. It refutes permanent left-to-right finalization but not a revisable active-set/PAVA-style method.

Iteration 20 promotes the strictly increasing-low case as solved locally. For \(0<\ell_1<\cdots<\ell_r<h\), the exact one-drought value is
\[
Z_{\rm inc}=\max_k
\left[
\left(1-\frac{\ell_k}{h}\right)S_k
+\sum_{j>k}\frac{\ell_k(h-\ell_j)}{h\ell_j}(S_j-S_{j-1})
\right],
\]
with the path-feasible \(S_j=[\ell_j(H+C_j)-hH]_+\). The proof combines a zero-pressure dual with a nested Hall scheduling inequality. The immediate writeup target is a polished theorem-proof block for LB-047, including the recurrence selecting \(k\), the Hall identity, and the strict-crossing \(\eta\)-implementation.

The next open local targets are now:

- nonmonotone lows, especially \(0.4,0.9,0.5\) and \(0.9,0.4,0.8\), where \(m_{i,j}\ne\ell_j\) and zero-pressure can fail;
- duplicate lows and zero-gap segments, where the strict-increase tail-weight proof has boundary cases;
- positive-pressure active sets after a Hall/canonical certificate fails outside the path-feasible increasing case;
- repeated droughts sharing the same resolving high, where high debt may carry across local one-drought certificates;
- an exact rational LP regression harness that outputs primal masses, dual weights, \(\psi_i\), active constraints, and residuals for every proposed formula.
